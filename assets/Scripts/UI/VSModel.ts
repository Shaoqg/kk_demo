...
import { getStrength, PetData, ElementType, getPetConfigById } from "../Config";
...
export default class VSModel extends ViewConnector {
    ...
    async startBattle() {
        ...
        this.selfInfo.AllStrength;

        this.opponentInfo.AllStrength;

        if (this.selfInfo.AllStrength > this.opponentInfo.AllStrength) {
    ...
    init(self:PetData[], opponent:PetData[]) {
        ...
        let selfStrengthInfo = this.updatePlayerInfo(selfNode, self);
        this.selfInfo = {
            node:selfNode,
            petDatas: self,
        ...
        let opponentStrengthInfo = this.updatePlayerInfo(opponentNode, opponent);
        this.opponentInfo = {
            node:opponentNode,
            petDatas: opponent,
    ...
    updatePlayerInfo(node:cc.Node,petDatas:PetData[]) {
        //updatepet
        petDatas.forEach((petData, i)=>{
            let petNode = cc.find(`pets/pet${i+1}`, node);
            this.updatePetInfo(petNode, petData);
        })

        //updatestrength
        return this.updateStrengthInfo(node, petDatas);
    }
    ...
    updateStrengthInfo(node:cc.Node,pets:PetData[], type = ElementType.fire) {
